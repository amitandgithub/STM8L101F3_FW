...
i2c::I2CStatus_t  i2c::XferPoll(uint8_t SlaveAddress,uint8_t* TxBuf, uint8_t TxLen, uint8_t* RxBuf, uint8_t RxLen, uint8_t RepeatedStart) // 462 bytes
{
  if(!TxBuf && !RxBuf)
...
i2c::I2CStatus_t i2c::XferIntr(Transaction_t* pTransaction) 
{
  ...
  m_MasterContext.SlaveAddress       = pTransaction->SlaveAddress;
  m_MasterContext.TxBuf->Buf              = pTransaction->TxBuf;
  m_MasterContext.TxBuf->Len            = pTransaction->TxLen;
  m_MasterContext.RxBuf->Buf             = pTransaction->RxBuf;
  m_MasterContext.RxBuf->Len              = pTransaction->RxLen;  
  m_MasterContext.RepeatedStart      = pTransaction->RepeatedStart;  
  m_MasterContext.pStatus            = pTransaction->pStatus;// ? pStatus : &m_I2CStatus; 
  m_MasterContext.XferDoneCallback   = pTransaction->XferDoneCallback;
  
  if(!m_MasterContext.pStatus)
  {
    m_MasterContext.pStatus = &m_I2CStatus;
  }
  
  if( (!m_MasterContext.TxBuf) && (!m_MasterContext.RxBuf) )
    return I2C_INVALID_PARAMS;
  
  if((m_MasterContext.TxBuf->Len == 0) || (m_MasterContext.RxBuf->Len == 0))
  {
    /* RepeatedStart is only valid for TX and Rx type transfer */
    m_MasterContext.RepeatedStart = 0;
  }
...
{            
  if(m_MasterContext.RxBuf->Len != 0)
  {              
    ...
    if(m_MasterContext.RepeatedStart)
    {
      /* Generate Start */
      GenerateStart();
  ...
  {                
    ...
    (*m_MasterContext.pStatus) = I2C_XFER_DONE;
    
    // Transaction ended here, call the completion callback
    if(m_MasterContext.XferDoneCallback)
      m_MasterContext.XferDoneCallback();  
...
void i2c::AF_Handler()
{
  ...
  if((m_I2CState == I2C_MASTER_TX)) 
  {
    /* Generate Stop */
    GenerateStop();    
    *(m_MasterContext.pStatus) = I2C_ACK_FAIL;    
    return;
  }
...
void i2c::RXNE_Handler()
{
  if(m_I2CState == I2C_MASTER_RX)
  {
    ...
    if(m_MasterContext.RxBuf->Len == 2U)
    {
      /* Read data from DR */
      I2C_BUF_BYTE_IN( m_MasterContext.RxBuf);
    ...
    else if(m_MasterContext.RxBuf->Len == 1U)
    {
      /* Read data from DR */
      I2C_BUF_BYTE_IN( m_MasterContext.RxBuf);
...
#else
    if(m_MasterContext.RxBuf->Len > 3U)
    {
      /* Read data from DR */
      I2C_BUF_BYTE_IN( m_MasterContext.RxBuf)                
    }
    else if( (m_MasterContext.RxBuf->Len == 2U) || (m_MasterContext.RxBuf->Len == 3U) )
    {
...
void i2c::TXE_Handler()
{
  ...
  if(m_I2CState == I2C_MASTER_TX)   
  {
    ...
    if(m_MasterContext.TxBuf->Len > 0)
    {
      I2C_BUF_BYTE_OUT( m_MasterContext.TxBuf);
...
void i2c::SB_Handler()
{
  ...
  if( m_I2CState == I2C_MASTER_TX )
  {
    I2C_DATA_REG = m_MasterContext.SlaveAddress & I2C_DIR_WRITE; 
...
#endif 
    I2C_DATA_REG = m_MasterContext.SlaveAddress | I2C_DIR_READ;    
...
void i2c::ADDR_Handler()
{
  ...
  if(m_I2CState == I2C_MASTER_RX)
  {
    if(m_MasterContext.RxBuf->Len == 1U)   
    {
      /* Clear ADDR flag */
      ClearADDR();
...
#ifndef I2C_RX_METHOD_1 
    else if(m_MasterContext.RxBuf->Len == 2U)   
    {
      /* Clear ADDR flag */
      ClearADDR();
  ...
  else if((m_I2CState == I2C_READY) || (m_I2CState == I2C_SLAVE_RX) || (m_I2CState == I2C_SLAVE_TX) )
  {
    ...
    {
      m_MasterContext.SlaveAddress = (I2C->OARL & 0xFE); // Bit 1-7 are address
    }
    else
    {
      m_MasterContext.SlaveAddress = (I2C->OARL & 0xFE); // Bit 1-7 are address
...
void i2c::BTF_Handler()
{
  ...
  else if(m_I2CState == I2C_MASTER_RX)
  {
    ...
    {                 
      ...
      {          
        I2C_LOG_STATES(I2C_LOG_STOP_TIMEOUT);
        (*m_MasterContext.pStatus) = I2C_STOP_TIMEOUT;                
      } 
      TxnDoneHandler(0); 
    }
    else 
...
void i2c::SwapBuf(i2cBufType_t BufType, i2cBuf_t* i2cBuf)
{
  i2cBuf_t* TempBuf = i2cBuf; 
  
  if(BufType == I2C_BUF_TYPE_SLAVE_RX)
  {
    i2cBuf = m_SlaveContext.RxBuf;
    m_SlaveContext.RxBuf = TempBuf;
    
    /* Enable data reception again after switching the buffer */
    //I2C_ENABLE_ACK();
  }
  else if(I2C_BUF_TYPE_SLAVE_TX)
  {
    i2cBuf = m_SlaveContext.TxBuf;
    m_SlaveContext.TxBuf = TempBuf;
  }  
  else if(I2C_BUF_TYPE_MASTER_RX)
  {
    i2cBuf = m_MasterContext.RxBuf;
    m_MasterContext.RxBuf = TempBuf;
  } 
  else if(I2C_BUF_TYPE_MASTER_TX)
  {
    i2cBuf = m_MasterContext.TxBuf;
    m_MasterContext.TxBuf = TempBuf;
  } 
  else
  {
    // Error !
