...
//      uint8_t*                  RxBuf; 
    ...
    void ClockEnable(){CLK->PCKENR |= ((uint8_t)CLK_Peripheral_SPI);}
    
    void ClockDisable(){CLK->PCKENR &= (uint8_t)(~(uint8_t)CLK_Peripheral_SPI);}
    ...
    void SetMode(uint8_t mode){ SPI->CR1 &= 0xFCU; SPI->CR1 |= mode&3; } // Shd not call when Comm is ongoing, register SPI_CR1 CPHA: bit 0, CPOL: bit 1
...
#define SPI_BYTE_IN_8_BIT(__BUF)      __BUF.RxBuf.Buf[__BUF.RxBuf.Idx] = SPI->DR;__BUF.RxBuf.Idx++
  
#define SPI_BYTE_OUT_8_BIT(__BUF)     SPI->DR = __BUF.TxBuf.Buf[__BUF.TxBuf.Idx++]
  
#define SPI_TX_DATA_AVAILABLE(__BUF)  (__BUF.TxLen > 0)
  
#define SPI_RX_DATA_AVAILABLE(__BUF)  (__BUF.RxLen > 0)
