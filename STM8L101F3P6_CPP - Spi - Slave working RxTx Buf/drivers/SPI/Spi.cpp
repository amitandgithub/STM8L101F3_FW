...
//    }
    ...
    Spi::SpiStatus_t Spi::XferPoll(Transaction_t const *pTransaction)
    {
      ...
      if(HAL::Spi::m_Transaction.TxBuf.Idx < HAL::Spi::m_Transaction.TxBuf.Len)
      {
        SpiStatus = TxPoll(pTransaction->TxBuf.Buf,pTransaction->TxBuf.Len);
      }
      
      if(HAL::Spi::m_Transaction.RxBuf.Idx < HAL::Spi::m_Transaction.RxBuf.Len)
      {
    ...
    {      
      ...
      {      
        if( (pTransaction == 0) || ((pTransaction->TxBuf.Buf == 0) && (pTransaction->RxBuf.Buf == 0))  )
        {                       
          return SPI_INVALID_PARAMS;                
        }        
    ...
    {  
      ...
      if( (m_SPIState == SPI_MASTER_TX_DMA) && (HAL::Spi::m_Transaction.RxBuf.Idx >= HAL::Spi::m_Transaction.RxBuf.Len) )
      {
        /* Load Tx DMA for generating clock for Rx*/ 
        LoadTxDmaChannel(m_Transaction.RxBuf,m_Transaction.RxLen);
