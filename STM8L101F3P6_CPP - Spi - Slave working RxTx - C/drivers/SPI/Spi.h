...
#define SPI_LOG_STATES_SIZE 500
  ...
  public:
    ...
    SpiStatus_t CheckAndLoadTxn(uint8_t* TxBuf, uint32_t TxLen,uint8_t* RxBuf, uint32_t RxLen,spiCallback_t XferDoneCallback);
    ...
    SpiStatus_t RxIntr(uint8_t* RxBuf, uint32_t RxLen,spiCallback_t XferDoneCallback);
...
#define SPI_DISABLE_ALL_INTERRUPTS()  SPI->ICR = 0
