...
INTERRUPT_HANDLER(SPI_IRQHandler, 26)
{
  ...
  if( SPI->SR & SPI_SR_RXNE )
  {
    ...
    else if(HAL::Spi::m_Transaction.SPIState == HAL::Spi::SPI_SLAVE_TXRX)
    {
      HAL::Spi::m_Transaction.RxBuf[HAL::Spi::m_Transaction.TxIdx] = SPI->DR;
      SPI->DR = HAL::Spi::m_Transaction.TxBuf[HAL::Spi::m_Transaction.TxIdx];
      HAL::Spi::m_Transaction.TxIdx++;
      
      if(SPI_TX_DATA_COMPLETE(HAL::Spi::m_Transaction))
      {               
        SPI_DISABLE_ALL_INTERRUPTS(SPI);
        HAL::Spi::m_Transaction.SPIState = HAL::Spi::SPI_TXRX_DONE;
        if(HAL::Spi::m_Transaction.XferDoneCallback) HAL::Spi::m_Transaction.XferDoneCallback(HAL::Spi::SPI_SLAVE_TXRX_DONE);     
      }  
