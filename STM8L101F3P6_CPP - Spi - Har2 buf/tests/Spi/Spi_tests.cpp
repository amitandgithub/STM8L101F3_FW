...
void SpiCallback(Spi::SpiStatus_t Status)
{
  ...
  if(Status == Spi::SPI_SLAVE_RX_DONE )
  {
    ...
    spi.m_Transaction.RxBuf.Idx = 0;      
    spi.m_Transaction.SPIState = HAL::Spi::SPI_SLAVE_RX;
    spi.EnableInterrupt(SPI_ICR_RXIE);
  }  
  SpiTxnCount++;
}
uint32_t Pc_Count;
...
//      Transaction.XferDoneCallback = SpiCallback;
      ...
      spi.SrartListeningRXIntr8bit(SPI_RxBuf,128); // 32bytes - 456 us; 64bytes - 911 us; 128bytes - 1.82 ms
      while(1) Pc_Count++;
    default: break;
    }
    delay_ms(100);
