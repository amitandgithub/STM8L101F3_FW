...
//    }
    ...
    Spi::SpiStatus_t Spi::XferPoll(Transaction_t const *pTransaction)
    {
      ...
      if(pTransaction->TxBuf.Len > pTransaction->TxBuf.Idx)
      {
        SpiStatus = TxPoll(pTransaction->TxBuf.Buf,pTransaction->TxBuf.Len);
      }
      
      if(pTransaction->RxBuf.Len > pTransaction->RxBuf.Idx)
      {
        SpiStatus =  RxPoll(pTransaction->RxBuf.Buf,pTransaction->RxBuf.Len);
      }
      return SpiStatus;
    }
    ...
    Spi::SpiStatus_t Spi::CheckAndLoadTxn(Transaction_t const *pTransaction)
    {
      ...
      if((pTransaction == 0) || ( (!pTransaction->TxBuf.Buf) && (!pTransaction->RxBuf.Buf) ))  return SPI_INVALID_PARAMS;   
    ...
    Spi::SpiStatus_t Spi::CheckAndLoadTxn(uint8_t* TxBuf, uint32_t TxLen,uint8_t* RxBuf, uint32_t RxLen,spiMasterCallback_t XferDoneCallback)
    {
      ...
      m_Transaction.TxBuf.Buf = TxBuf;
      m_Transaction.TxBuf.Len = TxLen;
      m_Transaction.RxBuf.Buf = RxBuf;
      m_Transaction.RxBuf.Len = RxLen;
      m_Transaction.XferDoneCallback = XferDoneCallback; 
      m_Transaction.ChipSelectPort = PORT_NONE;
    ...
    {
      if(m_Transaction.TxBuf.Len > m_Transaction.TxBuf.Idx)
      {    
    ...
    {
      if(m_Transaction.RxBuf.Len > m_Transaction.RxBuf.Idx)
      {    
    ...
    {    
      ...
      m_Transaction.TxBuf.Buf = Buf;
      m_Transaction.TxBuf.Len = Len; 
      m_Transaction.TxBuf.Idx = 0;
      SPI_BYTE_OUT_8_BIT(m_Transaction);         
    ...
    {     
      ...
      m_Transaction.RxBuf.Buf = Buf;
      m_Transaction.RxBuf.Len = Len; 
      m_Transaction.RxBuf.Idx = 0;
      m_Transaction.SPIState = SPI_SLAVE_RX;
      EnableInterrupt(SPI_ICR_RXIE);
    }
//    
//    void Spi::SrartListening(uint8_t* Buf, uint16_t Len)
//    {     
//      if(m_SPIState != SPI_READY) return SPI_BUSY;
//      
//      if((pTransaction == 0) || ( (!m_Transaction.TxBuf) && (!m_Transaction.RxBuf) ))  return SPI_INVALID_PARAMS;   
//      
//      memcpy(&m_Transaction,pTransaction,sizeof(Transaction_t));
//      
//      m_Transaction.SPIState = SPI_SLAVE_RX;
