...
INTERRUPT_HANDLER(SPI_IRQHandler, 26)
{
  ...
  {
    if(HAL::Spi::m_Transaction.SPIState == HAL::Spi::SPI_SLAVE_RX)
    {
      SPI_BYTE_IN_8_BIT(HAL::Spi::m_Transaction);
      HAL::Spi::m_Transaction.RxBuf.Idx++;
      
      if(HAL::Spi::m_Transaction.RxBuf.Len > HAL::Spi::m_Transaction.RxBuf.Idx)
      {               
        SPI_DISABLE_INTERRUPTS(SPI_ICR_TXIE | SPI_ICR_RXIE | SPI_ICR_ERRIE ); 
        HAL::Spi::m_Transaction.SPIState = HAL::Spi::SPI_READY;
        HAL::Spi::m_Transaction.RxBuf.Idx = 0;
        if(HAL::Spi::m_Transaction.XferDoneCallback) HAL::Spi::m_Transaction.XferDoneCallback(HAL::Spi::SPI_SLAVE_RX_DONE);     
      }   
    }
    else if(HAL::Spi::m_Transaction.SPIState == HAL::Spi::SPI_SLAVE_TX)
    {
      SPI_BYTE_OUT_8_BIT(HAL::Spi::m_Transaction);
      
      if(HAL::Spi::m_Transaction.TxBuf.Len > HAL::Spi::m_Transaction.TxBuf.Idx)
      {             
        HAL::Spi::m_Transaction.SPIState = HAL::Spi::SPI_READY;
        HAL::Spi::m_Transaction.TxBuf.Idx = 0;
        SPI_DISABLE_INTERRUPTS(SPI_ICR_TXIE | SPI_ICR_RXIE | SPI_ICR_ERRIE );            
